...
namespace Application{
	...
	//inline PROC __stdcall getProcAddresswglintf(LPCSTR name);/*{

		//return dll_opengl->get_ProcAddress(name);
	//};*/

	//inline void Atestfunc();
	template <typename T_swapBuffersFuncType, typename T_swapBuffers_class_reference, typename T_DRAW_STRUCTURE>
	GLMain<T_swapBuffersFuncType, T_swapBuffers_class_reference, T_DRAW_STRUCTURE> * setup_system_gl_opengl_layer(HWND native_window_handle,OpenGLContext*ctx);/* (HWND native_window_handle){
	...
	/*das inline hier ist nur n hack für diese scheiß c-compiler,wie kann man nur so was bauen,gucken ob ichs wegkriege */
	inline LRESULT CALLBACK Basic_WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam);/*{
		switch (message)
		{
	...
	inline void fullscreen(Windows::standard_window*window)/*{
...
#include "App_Initialize_Components.cpp"